        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
					...
					/* For Fronend */
					add_action( 'wp_head', array( &$this, '_outputFrontCss' ), 150 );
					add_action( 'wp_enqueue_scripts', array( &$this, '_outputFrontJs' ), 150 );
...
// set_global_variable()
            ...
            public function set_options( $value = '' ) {
                ...
                if ( ! empty ( $value ) ) {
					...
					if(!isset($value['report-brand-show'])):
						$value['report-brand-show']='';
					endif;
					if(!isset($value['report-map-comparable-show'])):
						$value['report-map-comparable-show']='';
					endif;
					if(!isset($value['report-comparable-list-show'])):
						$value['report-comparable-list-show']='';
					endif;
					if(!isset($value['report-property-detail-show'])):
						$value['report-property-detail-show']='';
					endif;
					if(!isset($value['report-photos-listing-show'])):
						$value['report-photos-listing-show']='';
					endif;
					if(!isset($value['report-comparative-analysis-show'])):
						$value['report-comparative-analysis-show']='';
					endif;
					if(!isset($value['report-analysis-analysis-show'])):
						$value['report-analysis-analysis-show']='';
					endif;
					
					$value['shortcode-example']='[property-search]';
...
// _admin_bar_menu()
			...
			/* Front End Css  */			
			public function _outputFrontCss() { 	
				wp_enqueue_style( $this->args['page_slug'].'-front-property-css', plugin_dir_url(dirname(__FILE__)) . 'public/css/front-property.css', array(), 'all' );
				//wp_enqueue_style( $this->args['page_slug'].'-front-bootstrap-css', plugin_dir_url(dirname(__FILE__)) . 'public/css/bootstrap.min.css', array(), 'all' );
			}
			/* Front End Js  */			
			public function _outputFrontJs() { 				
				wp_enqueue_script( $this->args['page_slug'].'-front-property-js', plugin_dir_url(dirname(__FILE__)) . 'public/js/property.js', array( 'jquery' ), $this->args['display_version'], false );
			}
